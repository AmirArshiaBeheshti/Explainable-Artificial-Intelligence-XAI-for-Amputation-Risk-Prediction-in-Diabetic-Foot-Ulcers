...
Original file is located at
    https://colab.research.google.com/drive/1lOsCXaLKDzJCrgDAGzZar4LDLNqXWvvA
"""
...
print("\n" + "="*80)
print("🔬 SOFTWARE VERSION INFORMATION (for Reproducibility)")
print("="*80)
...
# ================================================================================
output_dir = "/content/results_q1_revision/"
...
version_df.to_csv(os.path.join(output_dir, 'Software_Versions.csv'), index=False)

print(f"\n📁 Results will be saved to: {output_dir}")

# ================================================================================
# 5. DATA LOADING & QUALITY CHECKS
# ================================================================================
import os
import pandas as pd
...
print("\n" + "="*80)
print("📈 POWER ANALYSIS & EVENTS PER VARIABLE (EPV)")
print("="*80)

# حذف ستون 'CASE ID' از ویژگی‌ها
X = data.drop(['AMPUTATION', 'CASE ID'], axis=1, errors='ignore')
y = data['AMPUTATION']

# محاسبه تعداد نمونه‌ها و رویدادها
n_samples = len(data)
n_events = y.sum()
...
if epv < 10:
    print(f"  ⚠️ Warning: EPV < 10. Recommended minimum events: {n_predictors * 10}")
else:
    print(f"  ✓ EPV ≥ 10: Sample size is adequate")
...
print("\n" + "="*80)
print("📋 GENERATING BASELINE CHARACTERISTICS TABLE (TABLE 1)")
print("="*80)

# حذف 'CASE ID' از X در صورتی که هنوز باقی مانده
X = data.drop(['AMPUTATION', 'CASE ID'], axis=1, errors='ignore')
y = data['AMPUTATION']
...
print(f"✓ Baseline characteristics table created")
...
# ================================================================================
print("\n" + "="*80)
print("🎯 FEATURE SELECTION (Training Set Only - NO DATA LEAKAGE)")
print("="*80)

# Save target directory for results
output_dir = "/content/results_q1_revision"
os.makedirs(os.path.join(output_dir, "feature_selection"), exist_ok=True)

# استفاده مجدد از دسته‌بندی قبلی ولی فقط برای داده‌های آموزش
...
print("\n📌 Step 1: Univariate Feature Selection (p < 0.1)...")
...
print(f"✓ Selected {len(selected_features)} features with p < 0.1")
...
print("\n📌 Step 2: Correlation Analysis (|r| > 0.7)...")
...
if len(numeric_selected) > 1:
    ...
    if remove_corr:
        print(f"⚠️ Removing correlated features: {remove_corr}")
        selected_features = [f for f in selected_features if f not in remove_corr]

# 9.3 VIF Analysis
print("\n📌 Step 3: VIF Analysis (VIF > 10 removal)...")
numeric_selected = [f for f in selected_features if f in numeric_train]
...
if len(numeric_selected) > 1:
    ...
    while not stop and len(numeric_selected) > 1:
        ...
        if max_vif > 10:
            ...
            print(f"  🔄 Removing {feat} (VIF={max_vif:.2f})")
            numeric_selected.remove(feat)
            selected_features.remove(feat)
        else:
            stop = True

    vif_df.to_csv(os.path.join(output_dir, "feature_selection", "VIF_Report.csv"), index=False)
...
print(f"\n📊 Updated EPV after selection: {final_epv:.2f}")

# ================================================================================
# 10. MODEL DEFINITION WITH COMPREHENSIVE HYPERPARAMETER TUNING
# ================================================================================
print("\n" + "="*80)
print("🤖 MODEL DEFINITION & COMPREHENSIVE HYPERPARAMETER TUNING")
print("="*80)
...
print("\n🔧 Performing hyperparameter tuning with SMOTE (RandomizedSearchCV)...")
...
print("Available models after tuning:", tuned_models.keys())

# ================================================================================
# 11. 10-FOLD CROSS-VALIDATION (FINAL VERSION WITH UPDATED SAVE PATH)
# ================================================================================
print("\n" + "="*80)
print("🔄 10-FOLD CROSS-VALIDATION (with SMOTE)")
print("="*80)

# Create directories if not exist
output_dir = "/content/results_q1_revision"
performance_dir = os.path.join(output_dir, "model_performance")
os.makedirs(performance_dir, exist_ok=True)

cv_10fold = StratifiedKFold(n_splits=10, shuffle=True, random_state=RANDOM_SEED)
results = []

print("\n📊 Evaluating all models with 10-fold CV...")
...
save_path = os.path.join(performance_dir, 'CrossValidation_Results.csv')
results_df.to_csv(save_path, index=False)

print(f"\n💾 Cross-validation results saved to:\n  {save_path}")

print("\n📌 Ranking of Models based on 10-fold CV AUC:")
...
print("\n📊 Creating Stacking Ensemble (Top 3 Models)...")

# Select top 3 models by CV AUC (exclude any previous ensemble if exists)
base_model_results = [r for r in results if r['Model'] in tuned_models]
top_3_models = sorted(base_model_results, key=lambda x: x['Mean_AUC'], reverse=True)[:3]
top_3_names = [m['Model'] for m in top_3_models]

print(f"  Top 3 models: {top_3_names}")

# Prepare estimators for stacking
estimators = [(name, tuned_models[name]) for name in top_3_names]
...
# Save CV results to the specified folder
os.makedirs('/content/results_q1_revision', exist_ok=True)
results_df = pd.DataFrame(results).sort_values(by='Mean_AUC', ascending=False)
results_df.to_csv('/content/results_q1_revision/CV_Results_10Fold.csv', index=False)
...
# ================================================================================
print("\n" + "="*80)
print("🎯 TRAINING FINAL MODELS & TEST SET EVALUATION")
print("="*80)
...
test_df.to_csv(os.path.join(output_dir, 'Test_Set_Results.csv'), index=False)
...
# ================================================================================
print("\n" + "="*80)
print("📊 DELONG'S TEST FOR AUC COMPARISONS")
print("="*80)
...
# ================================================================================
# 15. ROC CURVES WITH CONFIDENCE INTERVALS
# ================================================================================
print("\n" + "="*80)
...
plt.title('ROC Curves - All Models with 95% Confidence Intervals',
          fontsize=16, fontweight='bold')
plt.legend(loc='lower right', fontsize=10)
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'ROC_All_Models_with_CI.png'),
            dpi=300, bbox_inches='tight')
plt.close()

print("✅ ROC curves with CIs saved!")
...
for name, y_prob in all_predictions.items():
    ...
    print(f"  {name:25s} Brier: {brier:.4f}, HL p-value: {hl_p_str}")
...
plt.title('Decision Curve Analysis - Clinical Range (0.05-0.60)',
          fontsize=16, fontweight='bold')
plt.legend(fontsize=10, loc='upper right')
plt.grid(alpha=0.3)
plt.xlim(0.05, 0.60)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'dca', 'DCA_All_Models.png'),
            dpi=300, bbox_inches='tight')
plt.close()

print("✅ DCA plot saved!")

# ================================================================================
# 18. THRESHOLD ANALYSIS
# ================================================================================
print("\n" + "="*80)
print("🎯 THRESHOLD ANALYSIS (Youden's J Index)")
print("="*80)
...
for name, y_prob in all_predictions.items():
    ...
    print(f"  {name:25s} Threshold: {optimal_threshold:.4f} "
...
# ================================================================================
# 20. LOGISTIC REGRESSION WITH ODDS RATIOS
# ================================================================================
print("\n" + "="*80)
...
# ================================================================================
# 19. SHAP ANALYSIS (FINAL VERSION)
# ================================================================================
print("\n" + "="*80)
print("🔍 SHAP ANALYSIS (FINAL VERSION)")
print("="*80)
...
for name in tree_models:
    ...
    # Extract classifier from pipeline if exists
    clf = model.named_steps['clf'] if hasattr(model, 'named_steps') else model

    try:
        explainer = shap.TreeExplainer(clf)

        # Use training data WITHOUT SMOTE
        ...
        plt.title(f"SHAP Beeswarm - {name}", fontsize=14, fontweight='bold')
        plt.tight_layout()
...
# ================================================================================
# Non-tree models + AdaBoost (KernelExplainer)
# ================================================================================
nontree_models = ['LogisticRegression', 'SVM', 'KNN', 'AdaBoost']

print("\n📊 SHAP for non-tree models (KernelExplainer)...")

# Representative background sample (100 instances via k-means)
...
# ================================================================================
# 21. COMPREHENSIVE SUMMARY REPORT
# ================================================================================
print("\n" + "="*80)
...
summary_report = f"""
{'='*80}
COMPREHENSIVE ANALYSIS REPORT (Q1 JOURNAL REVISION)
...
1. DATASET INFORMATION
   ...
   - Complete records: {quality_report['Complete_Records']}
   - Missing data: {quality_report['Missing_Percentage']:.2f}%
...
2. POWER ANALYSIS
   ...
   - Original predictors: {n_predictors}
   - Final selected predictors: {len(selected_features)}
   - Events Per Variable (EPV): {final_epv:.2f}
   - EPV Status: {'✓ Adequate (≥10)' if final_epv >= 10 else '⚠ May be insufficient (<10)'}

3. FEATURE SELECTION (NO DATA LEAKAGE)
   ✓ Performed on training set only
   - Univariate selection (p<0.1): {univariate_results['Selected_Raw'].sum()} features
   - After correlation removal (|r|>0.7): {len(selected_features) - len([f for f in selected_features if f in categorical_vars])} numeric features
   - After VIF removal (VIF>10): {len(selected_features)} final features
   - Features: {selected_features}

4. CLASS IMBALANCE HANDLING
   ✓ SMOTE (Synthetic Minority Oversampling Technique)
   - Applied during cross-validation and training
   - Integrated into all model pipelines
...
7. MODEL PERFORMANCE (Test Set)
{test_df.head(5)[['Model', 'Test_AUC', 'Test_F1', 'Test_Precision', 'Test_Recall']].to_string(index=False)}
...
12. BASELINE CHARACTERISTICS
   ...
   - See: baseline_characteristics/Table1_Baseline_Characteristics.csv
...
14. ADDRESSED REVIEWER CONCERNS
   ✓ Data leakage eliminated (feature selection after split)
   ✓ Power analysis performed (EPV calculated)
   ✓ All models hyperparameter tuned
   ✓ SMOTE applied for class imbalance
   ✓ DeLong's test for model comparisons
   ✓ Baseline characteristics table (Table 1)
   ✓ Bonferroni correction applied throughout
   ✓ Comprehensive SHAP analysis (100 background samples)
...
# ================================================================================
print("\n" + "="*80)
print("🎨 CREATING PUBLICATION FIGURE GRID")
print("="*80)
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'Publication_Figure_Grid.png'),
            dpi=300, bbox_inches='tight')
plt.close()

print("✅ Publication figure grid created!")

# ================================================================================
# COMPLETION MESSAGE
# ================================================================================
print("\n" + "="*80)
print("🎉 Q1 JOURNAL REVISION ANALYSIS COMPLETE!")
print("="*80)
print(f"\n📁 All results saved to: {output_dir}")
print("\n✅ ADDRESSED ALL MAJOR REVIEWER CONCERNS:")
print("  1. ✓ Data leakage eliminated")
print("  2. ✓ Power analysis (EPV) performed")
print("  3. ✓ All models tuned")
print("  4. ✓ SMOTE for class imbalance")
print("  5. ✓ DeLong's test for comparisons")
print("  6. ✓ Baseline characteristics table")
print("  7. ✓ Bonferroni corrections applied")
print("  8. ✓ Comprehensive documentation")
print("\n📄 Key Files Generated:")
print("  - COMPREHENSIVE_ANALYSIS_REPORT.txt")
print("  - Table1_Baseline_Characteristics.csv")
print("  - EPV_Analysis.csv")
print("  - DeLong_Test_Results.csv")
print("  - All performance metrics and visualizations")
print("\n" + "="*80)
...
zip_path = Path("/content/results_q1_revision_final")
...
if src_dir.exists():
    ...
    print("\n📦 Creating ZIP file...")
    shutil.make_archive(str(zip_path), format="zip", root_dir=str(src_dir.parent), base_dir=src_dir.name)
    ...
    try:
        ...
        print("\n💾 File ready for manual download from:", zip_file)